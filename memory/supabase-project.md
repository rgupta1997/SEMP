...
- Project ref: `pzpnldkustokmwzflnfl` (host `db.pzpnldkustokmwzflnfl.supabase.co`).
- Migrations live in `supabase/migrations/` and are applied with the **Supabase CLI** via `npx supabase db push --db-url "<conn-string>"` (no local `psql`; CLI v2.99.0). The DB password is NOT stored here — ask the user for the connection string each time.
