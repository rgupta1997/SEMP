...
// ---------- Notifications ----------
// `type` used to be enumerated here AND in a Postgres check constraint AND in
// packages/notifications/core/registry.ts - three lists nothing kept in sync,
// which is how 28 registered notification types went live rejected by the
// database on every attempt with nobody finding out until someone went
// looking. The registry is now the only list: its keys ARE the type
// (`NotificationTypeKey` in packages/notifications/core/registry.ts), checked
// by the compiler at every call site instead of enumerated a second time here.
