...
import { z } from 'zod';
import { AUDIENCES, MODULE_KEYS, MODULES, PERMISSION_CODES, permissionsByArea } from '@semp/shared';
...
export function makeOrgRolesRouter(prisma: Prisma): Router {
  ...
  router.post('/organizations/:id/roles', orgAdmin, validateBody(assignRoleSchema), asyncHandler(async (req, res) => {
    const { user_id, role_id } = req.body as z.infer<typeof assignRoleSchema>;
    ...
    const row = await prisma.user_org_roles.upsert({
      where: { user_id_organization_id_role_id: { user_id, organization_id: req.params.id, role_id } },
      update: {},
      create: { user_id, organization_id: req.params.id, role_id, assigned_by: req.user!.id },
    });
    ...
    await audit(prisma, req, {
      ...
      organizationId: req.params.id,
      summary: `Gave ${user.name} the ${role.name} role`,
      diff: { role: { from: null, to: role.name } },
    });
