...
interface CrudOptions {
  ...
  writeGuards?: RequestHandler[]; // applied only to create/update/delete
}
...
export function makeCrudRouter(delegate: Delegate, opts: CrudOptions): Router {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    const rows = await delegate.findMany({ where, orderBy: opts.orderBy, take, skip });
    res.json(rows);
  }));
  ...
  router.post('/', ...writeGuards, validateBody(opts.createSchema), asyncHandler(async (req, res) => {
