/*
 * THE ALL-SPORTS BENCH - one championship, every scored sport, awaiting fixtures.
 *
 *   npx tsx scripts/seed-all-sports-bench.ts seed      # build it
 *   npx tsx scripts/seed-all-sports-bench.ts logins    # print the logins again
 *   npx tsx scripts/seed-all-sports-bench.ts cleanup   # delete EXACTLY what was built
 *
 ...
 *    .seed-all-sports-manifest.json; cleanup deletes those ids and only those, in FK
...
const MANIFEST = path.join(HERE, '.seed-all-sports-manifest.json');

const PASSWORD = 'Bench@2026';
const DOMAIN = 'allsports.test';
const TAG = 'AllSports';
...
async function seed() {
  ...
  await prisma.championships.create({
    data: {
      id: champId,
      name: `${TAG} Bench · All-Sports Games 2026`,
      slug: `all-sports-bench-${run}`,
  ...
  console.log(`\nCleanup: npx tsx scripts/seed-all-sports-bench.ts cleanup`);
}
