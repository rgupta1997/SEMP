...
import type { ContestSpec, FormatTemplate, RubberSpec, ScoringMode, TieSpec } from '@semp/shared';
import { eventTemplateFor } from '@semp/shared';
import { sportDef, type SportDef } from './engine';

// Event (ranking) templates now live in @semp/shared so the API shares them; re-export
// so existing `from '.../templates'` imports (console, seed, setup) keep working.
export { eventTemplateFor };
