      ...
      <div>
        {loading ? <Card className="p-10"><Spinner label="Loading templates" /></Card> : (
          <div className="space-y-6">
            ...
            {shown.length === 0 && (
              <p className="rounded-xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400">
...
function TemplateCard({ t, selected, onSelect, onDelete }: {
  ...
  return (
    <div className="group relative">
      ...
      {draws.length > 0 && (
        <div className="pointer-events-none absolute left-1/2 top-full z-30 hidden w-72 -translate-x-1/2 translate-y-2 rounded-xl border border-slate-200 bg-white p-3 shadow-xl group-hover:block group-focus-within:block dark:border-slate-800 dark:bg-slate-900">
  ...
  return (
    <button type="button" onClick={onSelect} aria-pressed={selected}
      className={cn(
        ...
        selected
          ? 'border-brand-500 ring-2 ring-brand-500/60 dark:border-brand-400'
          : 'border-slate-300 hover:border-slate-400 dark:border-slate-800 dark:hover:border-slate-600',
      )}>
      <span className="rounded-xl border border-slate-200 p-3 text-slate-400 dark:border-slate-800">
        <Plus size={20} />
      </span>
