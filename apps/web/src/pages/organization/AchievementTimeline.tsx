...
function Entry({ item, side }: { item: Item; side: 'left' | 'right' }) {
  return (
      ...
      <div className={cn('ml-7 lg:ml-0', side === 'left' ? 'lg:col-start-1' : 'lg:col-start-3')}>
        <article className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          {/* The claim's own words if there are any. Otherwise just who it was for -
              ...
              anything the reader has not already read one line above. */}
          {item.detail ? (
...
export function AchievementTimeline() {
  ...
  const path = orgId ? `/organizations/${orgId}/achievements/timeline?${qs}` : null;
  const { data, isLoading } = useApi<Page>(path);
  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Achievement timeline</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">A chronological history of milestones and accolades.</p>
        </div>
      ...
      {isLoading && items.length === 0 ? <Skeleton className="h-96" /> : items.length === 0 ? (
        <EmptyState
          ...
          description="Milestones appear here the moment a result is locked, or when a claimed achievement is validated."
        />
      ) : (
        <div className="relative">
          {/* The spine. Left-aligned on narrow screens, centred once cards alternate. */}
          <span aria-hidden className="absolute bottom-0 left-[7px] top-0 w-px bg-slate-200 dark:bg-slate-700 lg:left-1/2" />
            ...
            {groups.map((g) => (
              <section key={g.month} className="grid gap-6">
                ...
                <ul className="grid gap-6">
                  {g.items.map((i) => { index += 1; return <Entry key={i.id} item={i} side={index % 2 === 0 ? 'left' : 'right'} />; })}
                </ul>
              </section>
            ))}
          </div>
