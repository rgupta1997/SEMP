...
// open step to where it's done. It's collapsible while in progress - never
// dismissed to nothing: the header (with a live progress count) always stays,
// so it can be re-opened. The collapsed/expanded choice is remembered per
// `storageKey`. Once complete there is nothing left to collapse or navigate to,
// so it drops the title and toggle entirely and just states the completion -
// permanently, not until the next dismiss.
export function GettingStarted({ title, subtitle, state, storageKey, completeNote }: {
  ...
  if (state.complete) {
    return (
      <Card className="border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">
