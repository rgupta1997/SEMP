...
export function NotificationBell() {
  ...
  return (
    <>
      ...
      {/* Scrim */}
      {open && <div className="fixed inset-0 z-40 bg-slate-900/50" onClick={() => setOpen(false)} />}
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
          <div className="text-sm font-bold text-slate-800 dark:text-slate-100">Notifications</div>
          <div className="flex items-center gap-1.5">
            {canPost && <Button size="sm" onClick={() => setComposing(true)}>+ New</Button>}
            <button
