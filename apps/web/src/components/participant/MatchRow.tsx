...
// `showEvent` adds the event name (used on the cross-event dashboard/list).
export function MatchRow({ match, showEvent = true }: { match: MatchSummary; showEvent?: boolean }) {
  const navigate = useNavigate();
  const score = scoreLine(match);
  const context = [showEvent ? match.event?.name : null, match.sport, match.round]
    .filter(Boolean)
    .join(' · ');
    ...
    >
      <div className="min-w-0">
        ...
        <div className="mt-0.5 truncate text-sm font-semibold text-slate-800 dark:text-slate-200">
          ...
          {match.opponent?.institution ? <span className="font-normal text-slate-400 dark:text-slate-500"> · {match.opponent.institution}</span> : null}
        </div>
